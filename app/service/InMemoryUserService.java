...
public class InMemoryUserService extends BaseUserService {
	public Logger.ALogger logger = play.Logger
			.of("application.service.InMemoryUserService");
	...
	public Identity doSave(Identity identity) {
		...
		if (found != null) {
			...
			User u = new User(String.valueOf(System.currentTimeMillis()),
					identity);
			users.put(u.id, u);
		}
	...
	public void doLink(Identity current, Identity to) {
		...
		if (target == null) {
			...
			throw new RuntimeException("Can't find a user for identity: "
					+ current.identityId());
		}
		if (!target.identities.contains(to))
